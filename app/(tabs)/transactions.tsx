...
export default function TransactionsScreen() {
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.filters}>{(['all', 'income', 'expense'] as Filter[]).map((item) => <Pressable accessibilityRole="button" accessibilityState={{ selected: filter === item }} key={item} onPress={() => setFilter(item)} style={[styles.filter, { backgroundColor: filter === item ? colors.primary : colors.surface, borderColor: filter === item ? colors.primary : colors.border }]}><Text style={[styles.filterText, { color: filter === item ? colors.onPrimary : colors.text }]}>{item[0].toUpperCase() + item.slice(1)}</Text></Pressable>)}</View>
